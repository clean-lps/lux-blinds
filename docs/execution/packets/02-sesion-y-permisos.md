...
1. Integrar la biblioteca auth ya elegida; no crear criptografía propia.
2. requireActor(request): Promise<Actor>; authorize(actor,action,resource): void; obtener organización desde sesión.
3. Login/logout con envelope, rate limit y cookies de contrato.
...
Credenciales inválidas: mensaje genérico; sesión expirada denegada; logout revoca; recurso ajeno 404, función prohibida 403; role/ownerId del cliente no elevan privilegios.

Trazabilidad: F-01; T-02,T-14.

## Ficha que coordinación debe completar antes de ejecutar

- Subpaso autorizado: P02-[A/B/C o único].
