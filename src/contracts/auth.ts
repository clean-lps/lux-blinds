...
export const RegisterSchema=z.strictObject({companyName:z.string().trim().min(1).max(160),contactName:z.string().trim().min(1).max(160),email:z.email(),phone:z.string().min(5).max(32),password:z.string().min(12).max(128),confirmPassword:z.string(),taxId:z.string().max(80).optional(),taxExempt:z.boolean(),certificateId:IdSchema.optional(),termsVersion:z.string().min(1),smsConsent:z.literal(false).default(false),verificationMethod:z.literal('email').default('email')}).refine(x=>x.password===x.confirmPassword,{path:['confirmPassword'],message:'Passwords must match'}).refine(x=>!x.taxExempt||!!x.certificateId,{path:['certificateId'],message:'Certificate required'});
export const VerifySchema=z.strictObject({challengeId:z.string().min(1),code:z.string().regex(/^\d{6}$/)});
...
export type ChallengeDTO={challengeId:string;expiresAt:string;resendAfter:string;maskedDestination:string;devCode?:string};
