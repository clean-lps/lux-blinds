...
import { AttachmentForbiddenError, AttachmentNotFoundError, UploadRejectedError, createUploadIntent } from '@/server/storage/service';
...
export async function POST(request: Request): Promise<NextResponse> {
  ...
  try {
    ...
    const message = status === 401 ? 'Authentication required' : status === 404 ? 'Not found' : status === 403 ? 'Forbidden' : status === 422 ? 'Upload rejected' : 'Unable to create upload intent';
